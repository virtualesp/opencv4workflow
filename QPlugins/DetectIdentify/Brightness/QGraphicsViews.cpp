...
QGraphicsViews::QGraphicsViews(QWidget* parent) : QGraphicsView(parent)
{
	...
	//显示清晰度
	qBrightnessValue = new QLabel(this);
	qBrightnessValue->setObjectName("BrightnessValue");
	qBrightnessValue->setStyleSheet("color:rgb(200,255,200); background-color:rgba(50,50,50,160); font: Microsoft YaHei;font-size: 15px;font-weight: bold;");
	qBrightnessValue->setVisible(false);
	qBrightnessValue->setFixedWidth(300);
	//显示区域
	fBrightnessWidget = new QWidget(this);
	fBrightnessWidget->setFixedHeight(25);
	fBrightnessWidget->setGeometry(2, 2, 300, 25);
	fBrightnessWidget->setStyleSheet("background-color:rgba(0,0,0,0);");
	auto fClarityLayout = new QHBoxLayout(fBrightnessWidget);
	fClarityLayout->setSpacing(0);
	fClarityLayout->setMargin(0);
	fClarityLayout->addWidget(qBrightnessValue);
	fClarityLayout->addStretch();
	connect(item, &ImageItem::BrightnessValue, this, [&](QString InfoVal) {
		qBrightnessValue->setText(InfoVal);
		});
	SetType(false, false);
}
...
void QGraphicsViews::SetType(bool InfoFlg, bool GrayValueFlg)
{
	...
	qBrightnessValue->setVisible(!InfoFlg);
}
