...
QGraphicsViews::QGraphicsViews(QWidget* parent) : QGraphicsView(parent)
{
	...
	//显示相似度
	qScoreValue = new QLabel(this);
	qScoreValue->setObjectName("ScoreValue");
	qScoreValue->setStyleSheet("color:rgb(200,255,200); background-color:rgba(50,50,50,160); font: Microsoft YaHei;font-size: 15px;font-weight: bold;");
	qScoreValue->setVisible(false);
	qScoreValue->setFixedWidth(300);
	//显示区域
	fScoreWidget = new QWidget(this);
	fScoreWidget->setFixedHeight(25);
	fScoreWidget->setGeometry(2, 2, 300, 25);
	fScoreWidget->setStyleSheet("background-color:rgba(0,0,0,0);");
	auto fClarityLayout = new QHBoxLayout(fScoreWidget);
	fClarityLayout->setSpacing(0);
	fClarityLayout->setMargin(0);
	fClarityLayout->addWidget(qScoreValue);
	fClarityLayout->addStretch();
	connect(item, &ImageItem::ScoreValue, this, [&](QString InfoVal) {
		qScoreValue->setText(InfoVal);
		});
	SetType(false, false);
}
...
void QGraphicsViews::SetType(bool InfoFlg, bool GrayValueFlg)
{
	...
	qScoreValue->setVisible(!InfoFlg);
}
