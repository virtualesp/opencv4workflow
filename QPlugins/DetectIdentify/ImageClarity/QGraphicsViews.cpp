...
QGraphicsViews::QGraphicsViews(QWidget* parent) : QGraphicsView(parent)
{
	...
	//显示清晰度
	qClarityValue = new QLabel(this);
	qClarityValue->setObjectName("ClarityValue");
	qClarityValue->setStyleSheet("color:rgb(200,255,200); background-color:rgba(50,50,50,160); font: Microsoft YaHei;font-size: 15px;font-weight: bold;");
	qClarityValue->setVisible(false);
	qClarityValue->setFixedWidth(300);
	//显示区域
	fClarityWidget = new QWidget(this);
	fClarityWidget->setFixedHeight(25);
	fClarityWidget->setGeometry(2, 2, 300, 25);
	fClarityWidget->setStyleSheet("background-color:rgba(0,0,0,0);");
	auto fClarityLayout = new QHBoxLayout(fClarityWidget);
	fClarityLayout->setSpacing(0);
	fClarityLayout->setMargin(0);
	fClarityLayout->addWidget(qClarityValue);
	fClarityLayout->addStretch();
	connect(item, &ImageItem::ClarityValue, this, [&](QString InfoVal) {
		qClarityValue->setText(InfoVal);
		});
	SetType(false, false);
}
...
void QGraphicsViews::SetType(bool InfoFlg, bool GrayValueFlg)
{
	...
	qClarityValue->setVisible(!InfoFlg);
}
